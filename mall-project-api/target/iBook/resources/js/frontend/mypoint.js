$(function() {
	...
	var pageSize = 10;
	// 获取用户各店铺积分信息的URL
	var listUrl = 'p/getUserRecodeList';
	var pageNum = 1;
	var searchInfo = '';
	// 预先加载20条
	...
	function addItems(pageSize, pageIndex) {
		...
		$.getJSON(url, function(data) {
			if (data.success) {
				$(".title").text("我的积分(总:" + data.totalPoint+")");
				// 获取总数
				maxItems = data.count;
				var html = '';
				data.userRecodeList.map(function(item, index) {
					html += '' + '<div class="card" data-book-id="'
							...
							+ '<div class="item-subtitle">章节名称：' + item.article.articleName + ' (积分:' + item.point + ')'
							+ '</div>' + '</div>' + '</li>' + '</ul>'
