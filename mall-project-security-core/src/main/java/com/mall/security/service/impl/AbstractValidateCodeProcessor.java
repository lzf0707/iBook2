...
import com.mall.security.service.ValidateCodeRepository;
...
public abstract class AbstractValidateCodeProcessor<C extends ValidateCode> implements ValidateCodeProcessor {
	...
	public void validate(ServletWebRequest request) {
		...
		//判断是否需要校验
		try {
			String needVerify = ServletRequestUtils.getStringParameter(request.getRequest(),
					 "needVerify");
			if(!Boolean.parseBoolean(needVerify)) { //判断需要验证码校验
				return;
			}
		} catch (ServletRequestBindingException e1) {
			// TODO Auto-generated catch block
			throw new ValidateCodeException("获取是否需要判断标识失败");
		}
